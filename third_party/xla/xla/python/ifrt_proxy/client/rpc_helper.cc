 ...
 public:
  ...
  void DoneProcessingResponse(int64_t seq_num) {
    ...
    while (!pending_seq_nums_.empty() &&
      ...
      auto it = pending_unregistration_.find(next_seq_num_to_process_);
      CHECK(it != pending_unregistration_.end());
      for (UserContextId user_context_id : it->second) {
        auto it2 = user_contexts_.find(UserContextId(user_context_id));
        CHECK(it2 != user_contexts_.end());
        if (--it2->second.ref_count == 0) {
          user_contexts_to_erase.push_back(
              std::move(it2->second.tracked_user_context));
          user_contexts_.erase(it2);
        }
      }
      pending_unregistration_.erase(it);
