...
TEST(ProfileGrpcTest, ProfileWithOverrideHostname) {
  ...
  config_value.set_string_value("testhost");
  (*request.mutable_opts()
        ->mutable_advanced_configuration())["override_hostname"] = config_value;

  tensorflow::ProfileResponse response;
  absl::Status status = ProfileGrpc(service_addr, request, &response);
  EXPECT_TRUE(status.ok());

  std::string expected_filename = "testhost.xplane.pb";
  std::string run_dir =
      ProfilerJoinPath(request.repository_root(), request.session_id());
  std::string expected_filepath = ProfilerJoinPath(run_dir, expected_filename);
