...
struct InterpolationSpecification {
  ...
  // for collective ops only: e.g. AG, AR
  std::optional<CollectiveDeviceList> collective_device_list;
  std::optional<GPUCommunicationType> collective_comm;
  // for p2p op only, e.g. collective permute.
...
absl::StatusOr<InterpolationSpecification> Spec(
  ...
  if (instr->opcode() == HloOpcode::kCollectivePermute) {
    ...
    auto permute_type = GetCollectivePermuteCostModelType(
    ...
    return InterpolationSpecification{/*opcode=*/cp->opcode(),
                                      ...
                                      /*collective_device_list=*/std::nullopt,
                                      /*collective_comm=*/std::nullopt,
  ...
  return InterpolationSpecification{
      ...
      /*collective_device_list=*/collective->device_list(),
      /*collective_comm=*/comm,
      /*permute_type=*/std::nullopt};
}
...
ConstructExactInterpolators(int num_devices_per_host,
  ...
  for (auto& profile : profiles.entries()) {
    ...
    if (spec.opcode == HloOpcode::kCollectivePermute) {
      exact_key = {
          /*opcode=*/spec.opcode,
          /*device_list=*/std::nullopt,
          /*permute_type=*/spec.permute_type,
          /*data_type=*/std::nullopt,
      };
    } else {
      exact_key = {
          /*opcode=*/spec.opcode,
          /*device_list=*/spec.collective_device_list,
          /*permute_type=*/std::nullopt,
          /*data_type=*/
...
ConstructExactNNInterpolators(int num_devices_per_host,
  ...
  for (auto& profile : profiles.entries()) {
    ...
    VLOG(10) << "  Spec: opcode=" << spec.opcode
             ...
             << ", data_type=" << spec.data_type << ", permute_type="
             << (spec.permute_type.has_value()
                     ? std::to_string(static_cast<int>(*spec.permute_type))
                     : "nullopt");
    ...
    VLOG(10) << "  Constructed exact_key: opcode=" << exact_key.opcode
             << ", permute_type="
             << (exact_key.permute_type.has_value()
                     ? std::to_string(static_cast<int>(*exact_key.permute_type))
                     : "nullopt")
             << ", data_type="
...
ConstructFallbackInterpolators(int num_devices_per_host,
  ...
  for (auto& profile : profiles.entries()) {
    TF_ASSIGN_OR_RETURN(InterpolationSpecification spec,
                        Spec(num_devices_per_host, profile, device_info));
    if (spec.opcode == HloOpcode::kCollectivePermute ||
        spec.opcode == HloOpcode::kCollectivePermuteStart) {
      // Collective-permute uses exact interpolation only.
      continue;
    }
    CHECK(spec.collective_comm.has_value());
    CollectiveInterpolator::FallbackInterpolatorKey key{
        /*opcode=*/spec.opcode,
        /*communication_type=*/spec.collective_comm.value(),
    };
