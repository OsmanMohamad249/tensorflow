...
bool IsFp8TypeOrFp8TensorType(mlir::Type t) {
  auto element_type = t;
  auto shaped_type = mlir::dyn_cast<mlir::ShapedType>(t);
  if (shaped_type) {
  ...
  return llvm::isa<mlir::Float8E5M2Type, mlir::Float8E4M3FNType,
                   ...
                   mlir::Float8E4M3B11FNUZType>(element_type);
}
...
class LowerTruncFOp : public mlir::OpRewritePattern<mlir::arith::TruncFOp> {
 ...
 private:
  mlir::LogicalResult matchAndRewrite(
    ...
    auto rounding_mode_attr = op.getRoundingModeAttr();

    if (!rounding_mode_attr) {
      return rewriter.notifyMatchFailure(
          op->getLoc(),
          "TruncFOp must have a rounding mode attribute to be lowered to "
          "FpToFpOp.");
    }

    auto rounding_mode = rounding_mode_attr.getValue();
